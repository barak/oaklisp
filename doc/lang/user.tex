...
something like this: {\tt\#<op 806>}.  This format indicates the type
...
locale.  For example, the above operation might print out as {\tt\#<op
...
\doc{Returns either variables or symbols containing {\em word},
depending on {\em place}, which can be a locale or \df{symbol-table}.
{\em place} defaults to \dffl{current-locale}.}
...
\doc{Puts a trace on the operation stored in {\em global-var}, causing
...
Objects can be examined in detail with \df{describe}
function, which prints the object and its type with
\dffl{fancy-references} turned on, and then prints the object's
internal state, organized as instance-variable blocks from the
object's various component types.  An object's internal state usually
contains anonymous objects whose printed representation includes weak
pointers which can be dereferenced using \df{object-unhash}.
Together, \df{describe} and \df{object-unhash} constitute a simple but
effective inspector.

\op{describe}{object}
\doc{Prints out lots of stuff about {\em object}.}

\op{object-unhash}{i}
\doc{Dereferences the weak pointer {\em i}.}
...
\op{ret}{n \dt args}
\doc{Invokes restart handler {\em n}, as specified by the list of
handlers printed out by a subordinate evaluation loop.  {\tt(ret 0)},
