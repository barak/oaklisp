...
\op{compile-file}{locale file-name $[$output-name$]$}
...
that the program's semantics are preserved.  The object is written to
\emph{output-name} if given, otherwise into the directory
\dffl{compiler-output-directory} if that is set, otherwise beside the
source.
