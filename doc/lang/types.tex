...
\ty{type}
\doc{This type is the top of the {\it is-a} hierarchy.  It is the
type of types, so new types are created by instantiating it.}

\ty{object}
\doc{This type is the top of the {\it subtype}\  hierarchy, and has no
...
not guaranteed to be unique.  Symbols {\it are} interned, though.}
...
\pr{is-a?}{object type}
\doc{Determines whether \df{object} is an instance of {\em type} or
one of its subtypes. {\tt (is-a? {\em object} object)} is always
true.}

\pr{subtype?}{type1 type2}
\doc{Determines whether {\em type1} is a subtype of {\em type2}.
...
\sform{quote}{object}
\doc{Returns {\em object} without evaluating it.}
...
\gv{nil}
\doc{The value of this is the empty list, written {\tt ()}.
Notice that \df{nil} itself is just a variable, so {\tt (eq? nil
'nil)} is false.}

\discuss{{\bf Note:} currently {\tt ()} is the same as {\tt \#f}, the
...
Some types are {\em coercable}, meaning that there is an operations
associated with that type that allows objects to be coerced to it.  To
create a coercable type, one instantiates \df{coercable-type} rather
than \df{type}.
...
string) '(\#\char`\\f \#\char`\\o \#\char`\\o))}{"foo"}.  The reader
will read {\tt frog} preceded by a control-y character as {\tt
(coercer frog)}; this was motivated by the fact that control-y prints
as $\rightarrow$ on both Macintosh$^{\mbox{tm}}$ and Symbolics
computers, giving coercion a pleasant syntax, \evto{($\rightarrow$
string '(\#\char`\\f \#\char`\\o \#\char`\\o))}{"foo"}.}
...
operation.  By default, {\tt(is-a?\ {\em foo} {\em bar})} implies that
\evto{((coercer {\em bar}) {\em foo})}{\em foo}}
...
difficult.  Programmers are left wondering ``Has a type based on {\em
foo} with {\em bar, baz} and {\em zonk} mixed in been created, if so
what's its name, and if not what should I name it and where should I
define it?''

Oaklisp's {\em mixin managers} take care of this problem.  When one
needs ``the type based on {\em foo} with {\em bar, baz} and {\em zonk}
...
\doc{This returns a composite type whose supertypes are {\em
type-list}.  {\em Mixin-manager} checks its cache, and if the
requested type is not found it creates a type with {\tt (make type '()
{\em type-list})}, caches it, and returns it.}
