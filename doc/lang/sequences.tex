...
\begin{figure}[h]
\centerline{\input{graphic-seqhier}}
...
\doc{Returns a sequence of the type of \emph{sequence1}.  The
arguments must all be of the same type, except that when the first is
a list the last one may be anything at all: as in R3RS, it becomes the
tail of the result, so \texttt{(append '(1 2) 3)} is
\texttt{(1 2 . 3)}.  A mismatch between any of the others is an
error.}
...
The same rule about the last argument applies here as in \df{append}.}
