...
\doc{Changes the value of {\em location} to {\em new-value}, which is then
returned as the value of the expression.

If {\em location} is a symbol, then it is interpreted as a variable
name.  The variable must have been previously defined in some lexical binding
contour or locale.

If {\em location} is a list, then it is interpreted as a reference to
a settable access operation.  For example, {\tt (set! (car foo) 'bar)}
means the same thing as {\tt (rplaca foo 'bar)} in Common Lisp.}
...
\doc{Returns a locative that points to {\em location}, which must be
...
\doc{Returns the contents of the location which is referenced by {\em
locative}.  Since \df{contents} is a settable operation, side effects
can be performed on locations through locatives.  For example, {\tt
(set! (contents (make-locative (car {\em foo}))) '{\em bar})} has the
same effect as {\tt (set! (car {\em foo}) '{\em bar})}.}
