...
\cline{1-3}
\em $w{-}1$ $w{-}2$ \ldots 11 10 9 8 & \em 7 6 5 4 3 2 & \em 1 0 &
...
Here $w$ is the \emph{word size}, the number of bits in a reference:
32 or 64, depending on how the emulator was built (see
section~\ref{sec:architectures}).  Fixnums thus have 30 or 62 bits.
...
Memory is a linear array of \emph{cells}, aligned words of the word
size.  These
...
\citet{SNYDER86}.} These semantics allow us to reference instance
...
always 0, and several are packed into each cell: two in a 32-bit
cell, and either two or four in a 64-bit one, as chosen when the
emulator is built (section~\ref{sec:architectures}).  Here is an
example of some stuff taken from the middle of a code vector, for two
...
From Oaklisp's point of view a machine is characterized by three
things:
\begin{itemize}
\item the \emph{word size}, the number of bits in a reference, 32 or
  64, which determines the fixnum range;
\item the number of 16-bit \emph{instructions packed into each cell}
  of a code vector: 2, or, with 64-bit cells, 4.  With two
  instructions in a 64-bit cell half of the cell is unused; four fill
  it, halving the size of code vectors, at the cost of bytecode that
  32-bit machines cannot use;
\item the \emph{byte order} of the machine, which decides the order in
  which the instructions in a cell lie in memory.
\end{itemize}

Instructions are packed into a cell in memory order: instruction $k$
of a cell occupies bits $16k$ to $16k+15$ on a little-endian machine,
and bits $w-16(k+1)$ to $w-16k-1$ on a big-endian one, so that the
emulator fetches successive instructions from successive 16-bit
units of memory.  (With two instructions in a 64-bit cell the
remaining 32 bits, the high half on a little-endian machine and the
low half on a big-endian one, are unused.)  An inline reference in
the code, such as a constant or a global variable cell, occupies a
whole cell and must be cell aligned, so the assembler pads the
instruction before it with noops up to the next cell boundary, and
the emulator skips that padding rather than executing it.

These properties are named \df{bc}$n$\df{-}$w$ for bytecode, where
$n$ is the number of instructions per cell and $w$ the word size, and
\df{bc}$n$\df{-e}$b$$w$ for a running system or a binary world image,
where $b$ is \df{l} or \df{b} for little or big endian:
\df{bc2-32}, \df{bc2-64} and \df{bc4-64} bytecode, and \df{bc2-el32},
\df{bc2-eb32}, \df{bc2-el64}, \df{bc2-eb64}, \df{bc4-el64} and
\df{bc4-eb64} systems.

What is portable between architectures:
\begin{itemize}
\item Compiled bytecode (\df{.oa} files) is independent of byte order,
  and in practice of word size too, since the compiler does not fold
  constants whose value would depend on it; the word size in its name
  only records the fixnum range that its integer constants are assumed
  to fit, so \df{bc2-32} bytecode can be loaded anywhere.  It is
  specific to the instruction packing, since that decides where the
  padding before each inline reference goes.
\item Cold world images (section~\ref{sec:worldfiles}) are
  independent of byte order but specific to word size and packing.
\item Binary world images are specific to all three.
\end{itemize}
Each of these files begins with a header line naming what it is for,
which the emulator, the loader and the linker check.

Within a running system, the variables \df{\%\%word-size} and
\df{\%\%instructions-per-ref}, set by the linker, describe the
system, and \df{host-architecture} in the language manual is
derived from them.  A system can generate code for another
architecture, which is how a system for a new one is bootstrapped.

\oop{\%big-endian?}
\doc{This returns the endianity of the machine that Oaklisp is running
on: true if the first instruction fetched from a cell is the more
significant one.}
...
invoked.  The \dfsw{{-}{-}trace-gc} \emph{level} switch makes it
report what it does.  The default size of new space is 128k
references; it can be altered with the \dfsw{{-}{-}size-heap}
\emph{size} switch, where \emph{size} is measured in units of 1024
references.  The operations \df{\%gc} and \df{\%full-gc} invoke the
garbage collector explicitly.  Programs that use weak pointers can be
effected by garbage collection; for details, see
Section~\ref{sec:weak}.

The \dfsw{{-}{-}predump-gc} switch (on by default) indicates that if
and when the world is dumped, and if Oaklisp terminates with an exit
code of zero, a full garbage collection should be performed.  In full
garbage collections preceding world dumps, the root set does not
include the stacks.
