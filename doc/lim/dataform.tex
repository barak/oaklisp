...
Consider an object of type {\em foo}, which is based on {\em bar} and
{\em baz}.  {\em Bar} had instance variables {\tt bar-1} and {\tt bar-2}, baz
has instance variables {\tt baz-1}, {\tt baz-2} and {\tt baz-3}, and {\em foo}
has instance variable {\tt foo-1}.  {\em Foo} inherits the instance
variables of the types it is based on, but methods defined for type
{\em foo} can not refer to these inherited variables.
...
variable frame at compile time.  Here is an instance of {\em foo} as
it might actually be stored in memory:

\begin{center}
\begin{tabular}{|c|} \hline
 reference to type {\em foo} \\ \hline\hline
 value of {\tt foo-1} \\ \hline\hline
 value of {\tt baz-1} \\ \hline
 value of {\tt baz-2} \\ \hline
 value of {\tt baz-3} \\ \hline\hline
 value of {\tt bar-1} \\ \hline
 value of {\tt bar-2} \\ \hline
\end{tabular}
\end{center}

Observe that instances of type {\em foo} are divided into contiguous
...
{\em stack segment} which is allocated on the heap.  These overflown
...
terms of {\em stack photos,} which are snapshots of the current state
...
facility provides with user with a second class {\em catch tag}, which
...
thrown to using the \df{throw} operation, as in {\tt (native-catch a
...
\dfsw{-h} {\em size} switch, where {\em size} is measured in bytes.  The
