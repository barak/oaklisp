...
\emph{non-error-lambda} is provided in which case it is called on that
result and the resultant value returned.  Both lambdas run outside the
handler this construct installs, so an error raised by either one is
not caught here.}
