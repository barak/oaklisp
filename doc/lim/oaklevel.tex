...
\lo{\%fluid}{symbol}
\doc{This looks {\em symbol} up on \df{fluid-binding-list}.  If it
is not found an error is signaled.  In contrast, {\tt (setter
...
leave it.  The wind/unwind action pairs are of the form {\tt ({\em
after} {\em before} . {\em saved-fluid-binding-list})} where {\em
before} and {\em after} are operations, guards to be called when
leaving and entering this dynamic context respectively, and {\em
saved-fluid-binding-list} is the appropriate value for
\df{fluid-binding-list} when calling these guard operations.}

\gv{\%wind-count}
\doc{To reduce \df{find-join-point}'s complexity from quadratic to
linear, we maintain \df{\%wind-count} $=$ {\tt (length \%windings)}.}
...
is identified by {\em string} in the list of choices printed out by
...
error occurs, an \df{error-return} form yields the value of {\em
body}.}

\mc{error-restart}{message let-clauses \dt body}
\doc{Acts like a \df{let}, binding the {\em let-clauses} as you would
expect, except that if an error occurs while evaluating {\em body},
the user is given the option of specifying new values for the
variables of the {\em let-clauses} and starting {\em body} again.
This is implemented with a \df{native-catch} and some tricky restart
handlers that get pushed onto {\tt (fluid restart-handlers)}.}
...
\doc{Evaluates {\em body}.  If an error which is a subtype of {\em
error-type} occurs, \df{\#f} is returned, unless {\em error-lambda} is
given, in which case it is called on the error object.  If no error
occurs then the result of evaluating {\em body} is returned, unless
{\em non-error-lambda} is provided in which case it is called on the
result of the evaluation of {\em body} within the context of of the
error handler, and the resultant value returned.}

\mc{bind-error-handler}{\lpar error-type handler\rpar \dt body}
 \doc{This binds a handler to errors which are subtypes of {\em
error-type}.  When such an error occurs, an appropriate error object
is created and {\em handler} is applied to it.}
...
handler, and after it stashes away the continuation it calls {\em
after-op} on {\em error}.  Of course, {\em after-op} should never
return.}

\op{invoke-in-error-context}{error operation}
\doc{Go back to the context in which {\em error} occured and invoke
{\em operation} there.}

\op{report}{error stream}
\doc{Write a human readable account of the error to {\em stream}.
Controlled studies have shown that error messages can never be too
verbose.}

\op{proceed}{error value}
\doc{Proceed from {\em error}, returning {\em value}.  Of course, it
is actually the call to \df{signal} that returns {\em value}.}


\op{signal}{error-type \dt args}
\doc{This signals creates an error of type {\em error-type} with
initialization arguments {\em args}.  It then scans down {\tt (fluid
error-handlers)} until it finds a type of error which is a supertype
of {\em error-type}, at which point it sends the corresponding handler
...
\framebox{\tt 0 0} in the tag field.  We could pack four seven bit
...
{\tt "Oaklisp Rules!"} as represented in memory.}
\begin{center}
...
\op{intern}{string}
\doc{Returns a symbol with print name {\em string} by looking it up
...
\doc{This flag is only relevent if {\tt (fluid print-escape)} is on.
With the value \df{t-compatible} then the empty symbol is printed as
{\tt \#[symbol ""]} and all other symbols requiring escaping are
printed with a {\tt \char`\\} character preceding every character of the
symbol.  With any other value, escaped symbols are delimited by {\tt
|} characters and internal characters {\tt \char`\\} and {\tt |} are
preceded by {\tt \char`\\}.}
...
\doc{Returns {\em value}.}

\op{listify-args}{operation \dt args}
\doc{Calls {\em operation} with a single argument, a list of {\em
args}.}
